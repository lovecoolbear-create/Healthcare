<template>
  <view>
    <!-- #ifdef H5 -->
    <DesktopProducts v-if="isDesktop" />
    <!-- #endif -->
    ...
    <view v-if="!isDesktop" class="mp-page-shell min-h-screen bg-transparent flex flex-col items-center justify-center p-10 text-center">
      ...
      <text class="text-slate-500 text-sm">产品库管理功能仅支持在 PC 端操作，以获得最佳体验。</text>
      <button
        @click="handleBack"
...
import { ref } from 'vue';
// #ifdef H5
import DesktopProducts from './components/DesktopProducts.vue';
// #endif
...
const handleBack = () => {
  ...
  uni.redirectTo({ url: '/pages/admin/settings/index' });
};

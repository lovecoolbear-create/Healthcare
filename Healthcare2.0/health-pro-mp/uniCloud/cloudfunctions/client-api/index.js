...
const verifyToken = async (token) => {
  ...
  try {
    ...
    return { valid: true, userId: userRes.data[0]._id, role: userRes.data[0].role };
  } catch (err) {
    ...
    switch (action) {
      ...
      case 'getAdminDashboardData': {
        ...
        for (const client of clientsRes.data) {
          ...
          // 【关键修复】只处理客户当前分配的有效方案
          // 获取客户当前分配的方案列表（支持多方案、单方案及对象/字符串的归一化）
      ...
      case 'getOwnProtocol': {
        ...
        // 过滤掉已停止的方案
        let activePlans = planRes.data.filter(p => !p.status || p.status === 'active');
          ...
          if (user && user.assigned_templates && user.assigned_templates.length > 0) {
            ...
            if (activeAssigned.length > 0) {
              ...
              activePlans = templatesRes.data.map((template, index) => ({
                ...
                plan_index: index,
                tasks: (template.products || template.items || []).map((product, pIndex) => ({
        ...
        if (activePlans.length > 0) {
          ...
          const combinedProtocol = {
            ...
            name: combinedName,
            status: 'active',
            items: mergeTasks(mergedTasks), // 【关键修复】应用智能合并
            date: today
          };

          console.log('✅ getOwnProtocol - combined name:', combinedName, ', protocols:', protocols.length);
      ...
      case 'getDailyPlan': {
        ...
        if (deduplicatedPlans.length > 0) {
          ...
          // 合并所有计划记录的 tasks，返回给小程序使用
          const aggregatedTasks = validPlansForClient.flatMap(p => p.tasks || []);
          const mergedTasks = mergeTasks(aggregatedTasks); // 【关键修复】应用智能合并
          ...
          return { 
            ...
            data: { 
              ...planRes.data[0], 
              tasks: mergedTasks, 
              plan_count: planRes.data.length,
      ...
      case 'stopProtocol': {
        ...
        if (protocol_id) {
          ...
          console.log('stopProtocol - 方案已停止:', protocol_id);
